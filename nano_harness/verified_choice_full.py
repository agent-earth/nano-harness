...
class VerifiedChoiceFullConfig:
    schema_version: str
    experiment_id: str
    parser_version: str
    manifest_path: str
    ...
    canary_pass_report_path: str
    canary_pass_report_sha256: str
    four_b_raw_path: str
...
def load_config(path: str | Path) -> VerifiedChoiceFullConfig:
    raw = json.loads(Path(path).read_text(encoding="utf-8"))
    expected = set(VerifiedChoiceFullConfig.__dataclass_fields__)
    if set(raw) != expected:
        raise ValueError("verified choice full config fields differ")
    config = VerifiedChoiceFullConfig(**raw)
    frozen = {
        "schema_version": "nano_harness_verified_choice_full_v1",
        "experiment_id": "anchored-v1-verified-choice-full-v1",
    ...
    for field, expected_value in frozen.items():
        if getattr(config, field) != expected_value:
            raise ValueError(
                f"verified choice full freezes {field}={expected_value}"
            )
    return config
...
def run(config: VerifiedChoiceFullConfig) -> dict[str, Any]:
    paths = {
        ...
        "canary_pass_report": Path(config.canary_pass_report_path),
    ...
    if case_manifest(cases) != committed_cases or len(cases) != 211:
        raise ValueError("verified choice full cases differ")
    committed_by_id = {
    ...
    canary_pass = json.loads(
        paths["canary_pass_report"].read_text(encoding="utf-8")
    )
    if (
        baseline_public.get("candidate", {}).get("correct") != 164
        or baseline_public.get("decision", {}).get(
            "passed_four_b_non_regression"
        )
        is not False
        or canary_pass.get("passed") is not True
        or canary_pass.get("decision", {}).get(
            "full_development_suite_allowed"
        )
        is not True
    ):
        raise ValueError("verified choice full staged receipts do not authorize")

    baseline_rows = _jsonl(paths["baseline_raw"])
    by_id = {str(row["case_id"]): row for row in baseline_rows}
    if len(baseline_rows) != 211 or set(by_id) != {
        case.case_id for case in cases
    }:
        raise ValueError("verified choice full raw case set differs")
    for case in cases:
        ...
        if (
            ...
            raise ValueError(
                f"verified choice full row parity failed: {case.case_id}"
            )
    ...
    result = {
        "schema_version": "nano_harness_verified_choice_full_result_v1",
