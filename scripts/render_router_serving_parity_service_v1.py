...
from nano_harness.router_serving_parity import load_config
...
RAW_HEALTH = (
    ROOT
    / "results/harness/qwen35-router-serving-parity-v1/services/"
    "models.json"
)
OUTPUT = (
    ROOT
...
def build_receipt() -> dict:
    ...
    if (
        ...
        raise ValueError("router serving parity preregistration differs")
    ...
    if (
        ...
        or by_id[config.served_models["original"]].get("parent")
        != config.served_models["base"]
        or by_id[config.served_models["remapped"]].get("parent")
        != config.served_models["base"]
    ):
        raise ValueError("router serving parity health differs")
    return {
        "schema_version": (
        ...
        "claim_boundary": (
            "This receipt proves only base plus original/remapped LoRA "
            "readiness. It contains no model generation or quality evidence."
        ),
    }
