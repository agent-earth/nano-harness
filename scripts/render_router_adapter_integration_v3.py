...
RAW = ROOT / "results/harness/qwen35-router-adapter-integration-v3/result.json"
PUBLIC_JSON = (
    ROOT / "docs/results/qwen35_router_adapter_integration_v3.public.json"
)
MARKDOWN = ROOT / "docs/results/qwen35_router_adapter_integration_v3.md"
...
def admission_gates(
    ...
    return {
        ...
        "router_outputs_parseable_160": all(
            receipt["router"]["label"] is not None
            for receipt in raw["receipts"].values()
        ),
        "router_a_recall_16": sum(
            receipt["router"]["expected_label"] == "A"
            and receipt["router"]["label"] == "A"
            for receipt in raw["receipts"].values()
        )
        == 16,
        "router_b_recall_16": sum(
            receipt["router"]["expected_label"] == "B"
            and receipt["router"]["label"] == "B"
            for receipt in raw["receipts"].values()
        )
        == 16,
        "router_c_recall_128": routing["negative_c_correct"] == 128,
        "each_c_subtype_recall_16": all(
            sum(
                case["family"] == subtype
                and raw["receipts"][case["case_id"]]["router"]["label"] == "C"
                for case in cases
            )
            == 16
        ...
        "positive_verified_executions_32": (
            routing["verified_executions"] == 32
        ),
        "positive_feedback_result_matches_32": sum(
...
def build_report() -> dict:
    ...
    if (
        ...
        != "nano_harness_router_adapter_integration_result_v3"
        ...
        or raw.get("v3_service_receipt") != service
    ):
        raise ValueError("router integration v3 result identity differs")
    for key in ("four_b_rows", "nine_b_rows", "candidate_rows"):
        rows = raw[key]
        if len(rows) != 160 or {row["case_id"] for row in rows} != set(by_id):
            raise ValueError(f"router integration v3 {key} differs")
    if set(raw["receipts"]) != set(by_id):
        raise ValueError("router integration v3 receipt identities differ")
    ...
    for case_id, receipt in raw["receipts"].items():
        ...
        if (
            ...
            raise ValueError("router integration v3 receipt differs")
    ...
    if (
        ...
        != raw["arms"]["four_b_router_adapter_v3"]
    ):
        raise ValueError("router integration v3 aggregate boundary differs")
    ...
    return {
        "schema_version": (
            "nano_harness_router_adapter_integration_public_v3"
        ),
        ...
        "data": {
            ...
            "family_counts": {
                family: (
                    config.positive_cases_per_family
                    if family in POSITIVE_FAMILIES
                    else config.negative_cases_per_subtype
                )
                for family in all_families
            },
        ...
        "decision": {
            ...
            "adapter_integration_v3_admitted": admitted,
            ...
            "integration_v3_rerun_allowed": False,
            "canary_allowed": False,
            "holdout_allowed": False,
            "training_or_rl_allowed": False,
            "further_tuning_allowed": False,
            "next_action": (
                "Pre-register one benchmark-agnostic router treatment on the "
                "frozen matched local suite; do not generate benchmark "
                "outputs before the treatment contract is committed."
                if admitted
                else
                "Reject this router treatment. Do not rerun or tune V1, V2, "
                "or V3."
            ),
        },
        ...
        "claim_boundary": (
            ...
            "canary, holdout, training, RL, or final model-superiority evidence."
        ),
    }
...
def render_markdown(report: dict) -> str:
    ...
    verdict = (
        "ADMIT"
        if decision["adapter_integration_v3_admitted"]
        else "REJECT"
    )
    return f"""# Qwen3.5 Router Adapter Integration v3 Result
